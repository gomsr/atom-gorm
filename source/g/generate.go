...
func G2(outputDir, relationYaml string, m *gconfig.DbServer, useTenant bool, opts ...gen.ModelOpt) (*gen.Generator, *gorm.DB) {
	var dialector gorm.Dialector
	switch m.DbType {
	case gconfig.DbMysql:
		dialector = mysql.Open(m.Mysql.Dsn())
	case gconfig.DbPgsql:
		dialector = postgres.Open(m.Pgsql.Dsn())
	default:
		panic("unknown db type")
	}
